...
function clioErrorTypeToString(int $type): string
{
    return match ($type) {
        E_ERROR => 'E_ERROR',
        E_WARNING => 'E_WARNING',
...
function clioGetErrorDumpDir(): ?string
{
    $candidates = [
        __DIR__ . '/../data/php_errors',
        __DIR__ . '/errors/php_errors',
        rtrim(sys_get_temp_dir(), '\\/') . '/clio_php_errors',
    ];
...
function clioWriteErrorDump(string $kind, string $message, string $file, int $line, ?int $severity = null, array $context = []): void
{
    $dir = clioGetErrorDumpDir();
    if ($dir === null) {
        error_log('[Clio error dump] Failed to resolve writable dump directory');
        return;
    }
    ...
    $content = [
        ...
        'file=' . $file,
        'line=' . (string) $line,
        'severity=' . ($severity !== null ? clioErrorTypeToString($severity) : ''),
    ...
    if ($ok === false) {
        error_log('[Clio error dump] Failed writing dump file: ' . $path);
    }
}
...
set_error_handler(static function (int $severity, string $message, string $file, int $line): bool {
    ...
    clioWriteErrorDump('php_error', $message, $file, $line, $severity);
    return false;
});
